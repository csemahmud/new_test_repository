...
includes_app.controller('users_ctrl', function($scope) {
    $scope.first_name = '';
    $scope.last_name = '';
    $scope.email = '';
    $scope.password1 = '';
    ...
    $scope.validate_user = function() {
        ...
        if(!$scope.first_name.length || !$scope.last_name.length ||
            ...
            $scope.incomplete = true;
            //alert('first_name:= ' + $scope.first_name + '; Length:= ' + $scope.first_name.length);
                } else {
                    $scope.incomplete = false;
                }
    };
    ...
    $scope.save_user = function() {
        ...
        $scope.password1 = '';
        $scope.password2 = '';
        //alert('Saving User : {user_id: ' +$scope.users[$scope.users.length-1].user_id
                //+ ', first_name: ' + $scope.users[$scope.users.length-1].first_name 
                //+ ', last_name: ' + $scope.users[$scope.users.length-1].last_name
                //+ ', email: ' + $scope.users[$scope.users.length-1].email);
    };
});